// api/analyze.js
// Bio-Chef Pro — AI Food Analysis Endpoint
// ─────────────────────────────────────────
...
1. AÇLIK KRONOMETRESİ: Glisemik indeks + sindirim süresine göre tahmini acıkma süresi (örn: “2-3 saat”).
1. ÜRÜN ÖZETİ: Gıdanın tam adı, tahmini porsiyon gramajı.
1. BESİN DEĞERLERİ: Kalori (kcal) · Protein (g) · Karbonhidrat (g) · Yağ (g) — sadece rakamlar.
1. HEDEF ANALİZİ: Kullanıcının kilo hedefine uygunluk değerlendirmesi.
1. NET TAVSİYE: Tüketim onayı/reddi ve 1-2 cümle profesyonel görüş.
...
if (!openaiRes.ok) {
  ...
  return res.status(502).json({ error: 'Yapay zeka servisi yanıt vermedi. Tekrar deneyin.' });
}
...
if (!analysis) {
  console.error('[BioChef] Boş yanıt:', JSON.stringify(data));
